# 宫颈癌 MDT AI Agent (Cervical Cancer MDT AI Agent)

这是一个基于 **LangGraph** 和 **Streamlit** 构建的多智能体（Multi-Agent）系统，旨在模拟真实的宫颈癌多学科会诊（MDT, Multi-Disciplinary Team）流程。该系统利用 Google Gemini 大模型（通过 LangChain 接入），为宫颈癌患者提供综合的诊疗建议。

## 项目特点

- **多学科协同**：模拟真实的 MDT 流程，包含超声科、放射科、病理科、妇科和肿瘤内科的虚拟专家。
- **动态工作流**：基于 LangGraph 构建的状态图，支持动态选择参与会诊的科室。诊断科室并行处理，随后汇总至妇科和肿瘤内科，最终由首席协调员（Coordinator）出具最终极简版 MDT 综合诊疗方案。
...
3. **妇科评估**：妇科节点汇总诊断阶段的结果，给出宫颈癌分期倾向、局部治疗路径与手术/同步放化疗相关建议。
4. **内科评估**：肿瘤内科节点结合病理和妇科意见，给出系统治疗与后续综合治疗建议。
